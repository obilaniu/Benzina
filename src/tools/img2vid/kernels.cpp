...
extern int  i2vFilter           (UNIVERSE*        u){
#if 0
	printf("%s\n", av_get_pix_fmt_name((AVPixelFormat)u->filt.in.frame->format));
	printf("%d\n", av_pix_fmt_count_planes((AVPixelFormat)u->filt.in.frame->format));
	printf("%dx%d\n", u->filt.in.frame->width, u->filt.in.frame->height);
	printf("%p\n", u->filt.in.frame->data);
	printf("\t%p  +  %d\n", u->filt.in.frame->data[0], u->filt.in.frame->linesize[0]);
	printf("\t%p  +  %d\n", u->filt.in.frame->data[1], u->filt.in.frame->linesize[1]);
	printf("\t%p  +  %d\n", u->filt.in.frame->data[2], u->filt.in.frame->linesize[2]);
	printf("%s\n", av_get_pix_fmt_name((AVPixelFormat)u->filt.out.frame->format));
	printf("%d\n", av_pix_fmt_count_planes((AVPixelFormat)u->filt.out.frame->format));
	printf("%dx%d\n", u->filt.out.frame->width, u->filt.out.frame->height);
	printf("%p\n", u->filt.out.frame->data);
	printf("\t%p  +  %d\n", u->filt.out.frame->data[0], u->filt.out.frame->linesize[0]);
	printf("\t%p  +  %d\n", u->filt.out.frame->data[1], u->filt.out.frame->linesize[1]);
	printf("\t%p  +  %d\n", u->filt.out.frame->data[2], u->filt.out.frame->linesize[2]);
#endif
	dim3 Dg, Db;
	...
	texObj2 = 0;
	err = cudaSetDevice(0);
	err = cudaMalloc(&cudaIPlane0, u->filt.in.frame->width *u->filt.in.frame->height /1/1);
	err = cudaMalloc(&cudaIPlane1, u->filt.in.frame->width *u->filt.in.frame->height /1/1);
	err = cudaMalloc(&cudaIPlane2, u->filt.in.frame->width *u->filt.in.frame->height /1/1);
	err = cudaMalloc(&cudaOPlane0, u->filt.out.frame->width*u->filt.out.frame->height/1/1);
	err = cudaMalloc(&cudaOPlane1, u->filt.out.frame->width*u->filt.out.frame->height/2/2);
	err = cudaMalloc(&cudaOPlane2, u->filt.out.frame->width*u->filt.out.frame->height/2/2);
	err = cudaMemset(cudaOPlane0,   0, u->filt.out.frame->width*u->filt.out.frame->height/1/1);
	err = cudaMemset(cudaOPlane1, 128, u->filt.out.frame->width*u->filt.out.frame->height/2/2);
	err = cudaMemset(cudaOPlane2, 255, u->filt.out.frame->width*u->filt.out.frame->height/2/2);
	err = cudaMemcpy2D(cudaIPlane0, u->filt.in.frame->width, u->filt.in.frame->data[0], u->filt.in.frame->linesize[0], u->filt.in.frame->width, u->filt.in.frame->height, cudaMemcpyHostToDevice);
	err = cudaMemcpy2D(cudaIPlane1, u->filt.in.frame->width, u->filt.in.frame->data[1], u->filt.in.frame->linesize[1], u->filt.in.frame->width, u->filt.in.frame->height, cudaMemcpyHostToDevice);
	err = cudaMemcpy2D(cudaIPlane2, u->filt.in.frame->width, u->filt.in.frame->data[2], u->filt.in.frame->linesize[2], u->filt.in.frame->width, u->filt.in.frame->height, cudaMemcpyHostToDevice);
	...
	resDesc0.res.pitch2D.height       = u->filt.in.frame->height;
	resDesc0.res.pitch2D.width        = u->filt.in.frame->width;
	resDesc0.res.pitch2D.pitchInBytes = u->filt.in.frame->linesize[0];
	...
	resDesc1.res.pitch2D.height       = u->filt.in.frame->height;
	resDesc1.res.pitch2D.width        = u->filt.in.frame->width;
	resDesc1.res.pitch2D.pitchInBytes = u->filt.in.frame->linesize[1];
	...
	resDesc2.res.pitch2D.height       = u->filt.in.frame->height;
	resDesc2.res.pitch2D.width        = u->filt.in.frame->width;
	resDesc2.res.pitch2D.pitchInBytes = u->filt.in.frame->linesize[2];
	...
	int cropX = u->tx.crop.l, cropY = u->tx.crop.t,
	    cropW = u->filt.in.frame->width -u->tx.crop.l-u->tx.crop.r,
	    cropH = u->filt.in.frame->height-u->tx.crop.t-u->tx.crop.b,
	    dstW  = u->filt.out.frame->width,
	    dstH  = u->filt.out.frame->height;
	Db.x = 32;
	Db.y = 32;
	Db.z =  1;
	...
	<<<Dg, Db>>>(
	            ...
	            cudaOPlane1,
	            cudaOPlane2,
	            u->filt.out.frame->height,
	            u->filt.out.frame->width,
	            u->tx.embed.x,   u->tx.embed.y, u->tx.embed.w, u->tx.embed.h,
	            cropX,           cropY,         cropW,         cropH,
	            u->tx.scale.x,   u->tx.scale.y,
	            1.0f,            1.0f,
	            1.0f,            1.0f,
	            1.0f,            1.0f,
	            u->tx.lumaoff.x, u->tx.lumaoff.y,
	            u->tx.lumaoff.x, u->tx.lumaoff.y,
	            u->tx.lumaoff.x, u->tx.lumaoff.y,
	            texObj0,
	            texObj1,
	...
	err = cudaMemcpy2D(u->filt.out.frame->data[0], u->filt.out.frame->linesize[0], cudaOPlane0, u->filt.out.frame->width/1, u->filt.out.frame->width/1, u->filt.out.frame->height/1, cudaMemcpyDeviceToHost);
	err = cudaMemcpy2D(u->filt.out.frame->data[1], u->filt.out.frame->linesize[1], cudaOPlane1, u->filt.out.frame->width/2, u->filt.out.frame->width/2, u->filt.out.frame->height/2, cudaMemcpyDeviceToHost);
	err = cudaMemcpy2D(u->filt.out.frame->data[2], u->filt.out.frame->linesize[2], cudaOPlane2, u->filt.out.frame->width/2, u->filt.out.frame->width/2, u->filt.out.frame->height/2, cudaMemcpyDeviceToHost);
